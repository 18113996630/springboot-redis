...
public class RedisLock {
	...
	public boolean lock(String key,String value){
		if(stringRedisTemplate.opsForValue().setIfAbsent(key,value)){//对应setnx命令
			//可以成功设置,也就是key不存在
			System.out.println("上一个线程把锁释放之后我又加上锁了，值为："+value);
			return true;
		}
		//判断锁超时 - 防止原来的操作异常，没有运行解锁操作  防止死锁
		String currentValue = stringRedisTemplate.opsForValue().get(key);
		System.out.println("现在redis中还有值，锁是否过期："+(Long.parseLong(currentValue) < System.currentTimeMillis()));
		//如果锁过期
		if(!StringUtils.isEmpty(currentValue) && Long.parseLong(currentValue) < System.currentTimeMillis()){//currentValue不为空且小于当前时间
			...
			System.out.println("*********进行重新设置value值，value"+value+"*********");
	...
	public void unlock(String key,String value){
		try {
			...
			if(!StringUtils.isEmpty(currentValue) && currentValue.equals(value) ){
				System.out.println("解锁：：：******参数："+value+"*******redis值："+currentValue+"*****************");
